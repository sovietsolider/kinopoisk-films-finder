import {
  FilmsFilterType,
  FilmsFilterToServer,
} from '@/components/routes/Films/components/FilmsFilter/types';
...
export class FilmsAdapter {
  public static filmsFilterToServer(
    limit: number,
    page: number,
  ...
  public static filterFromQuery(params: URLSearchParams): FilmsFilterType {
    ...
    resFilter.ageRating = queryFilter.ageRating?.split('-').map((d: string) => Number(d)) ?? [
      0, 18,
    ];
