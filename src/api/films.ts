...
export default class FilmsAPI {
  ...
  public static async getFilmsByName(name: string, limit: number, page: number) {
    return http.get(
      `/v1.4/movie/search?${name.length > 0 ? `query=${encodeURIComponent(name)}&` : ''}limit=${limit}&page=${page}`
    );
  }
  ...
  public static async getFilmImages(
    id: number,
  ...
  public static async getFilmSeasons(id: number, limit: number, page: number) {
    return http.get(
      `/v1.4/season?limit=${limit}&page=${page}&movieId=${id}&sortField=number&sortType=1`
    );
  }
  ...
  public static async getFilmsSeasonsNames(id: number, limit: number, page: number) {
    return http.get(
      `/v1.4/season?limit=${limit}&page=${page}&movieId=${id}&sortField=number&sortType=1&selectFields=name&selectFields=number`
    );
  }
